          ...
          <div className='max-w-[400px] w-full px-6 py-6'>
               <div className=''>
                    <figure className='bg-[#030303] p-1 max-w-[60px] mx-auto rounded-xl'>
                         <Image src={copan?.logo} alt={copan?.title} width={50} height={50} />
                    </figure>
                    <Heading className="text-center mt-3">Take {copan.off}% Off with {copan.siteName} <br /> Promo Code</Heading>
                    <div className='mt-7 flex mb-4'>
                         <input type={`text`} value={copan.coupon} ref={inputRef} className="border outline-none p-2 flex-1 text-gray-400 rounded-bl-md rounded-tl-md" />
                    ...
                    <Link href={copan.siteURL} target='_blank'>
                         <Button color='primary' fullwidth variants='primary' size='medium' rounded>Use at {copan.siteName}</Button>
                    </Link>
                    <p className="text-sm mt-3 text-center px-1">Copy the code, then go to <strong>{copan.siteName}</strong> and paste it in during checkout. Your savings will be applied!</p>
